...
class Home extends React.Component {
  ...
  onRangeChange(changeResult) {

    console.log(changeResult);
    const { minValue, maxValue } = changeResult;
    this.rangeChangedDataGdp = JSON.parse(JSON.stringify(this.changedDataGdp));
    console.log(this.rangeChangedDataGdp);
    for (let i = 1; i < this.rangeChangedDataGdp.length; i++) {
      if (this.rangeChangedDataGdp[i][0] === minValue) {
        this.rangeChangedDataGdp.splice(1, i);
      }
      if (this.rangeChangedDataGdp[i][0] === maxValue) {
